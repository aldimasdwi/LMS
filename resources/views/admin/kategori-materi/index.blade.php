      ...
      <div class="card-body table-responsive">
        <table id="dataTable1" class="table table-bordered table-hover">
          ...
          <tbody>
            ...
            @foreach($kategoriMateri as $kategoriMat)
            <tr>
              <td>{{ $no++ }}</td>
              <td><a href="{{ route('admin.kategori-materi.show', $kategoriMat->slug) }}">{{ $kategoriMat->nama_kategori
                  }}</a></td>
              <td>{{ $kategoriMat->deskripsi }}</td>
              <td>{{ $kategoriMat->jurusan->name }}</td>

              <td>
                <div class="row ml-2">
                  <a href="{{ route('admin.kategori-materi.edit',$kategoriMat->slug) }}"
                    class="btn btn-primary btn-sm"><i class="fas fa-edit fa-fw"></i></a>

                  <form method="POST" action="{{ route('admin.kategori-materi.destroy',$kategoriMat->slug) }}">
                    @csrf
                    @method('DELETE')
